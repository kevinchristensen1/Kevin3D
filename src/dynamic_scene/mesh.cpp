...
void Mesh::linearBlendSkinning(bool useCapsuleRadius) {
  ...
  Vector3D skeletonBindPos, meshBindPos, skeletonNewPos, ax, newVertexPosition;
  Joint *j;
  Matrix4x4 bindT, T;
  vector<LBSInfo> lbsinfo;
  int numJoints = skeleton->joints.size();
  lbsinfo.resize(numJoints);
  
  double distBeg, distSeg, distEnd, dotProduct, wij, sumWeights;
  int i, count; //for lbsinfo structure

  for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); v++) {
    i = 0;
    count = 0;
    sumWeights = 0.;
    newVertexPosition = Vector3D(0.,0.,0.);
    meshBindPos = v->bindPosition;

    for (vector<Joint*>::iterator it = skeleton->joints.begin() ; it != skeleton->joints.end(); ++it, i++)
    { 
      j = *it;
      bindT = j->getBindTransformation();
      bindT = bindT.inv();

      T = j->getTransformation() * j->SceneObject::getTransformation();
      
      skeletonBindPos = bindT * meshBindPos; //now in joint j's coord frame

      skeletonNewPos = T * skeletonBindPos; //transform to world coord's.
      
      ax = j->axis;
      dotProduct = dot(skeletonBindPos, ax);


      // It is closest to beginning of line segment
      if(dotProduct <= 0.)
        lbsinfo[i].distance = skeletonBindPos.norm();
      // It is closest to end of line segment
      else if (dotProduct >= ax.norm() )
        lbsinfo[i].distance = (skeletonBindPos - ax).norm();
      // Otherwise it is closest to a point on line segment
      //distSeg = ( skeletonBindPos - dot(skeletonBindPos, ax) * ax.unit() ).norm(); //distance to segement
      else
        //lbsinfo[i].distance = cross(skeletonBindPos-ax,ax).norm() / ax.norm();
        lbsinfo[i].distance = ( skeletonBindPos - ( dot(skeletonBindPos, ax) * ax.unit() ) ).norm();


      lbsinfo[i].distance = max(lbsinfo[i].distance, EPS_D); //prevent numerical instabilties
      lbsinfo[i].blendPos = skeletonNewPos;
      //if( lbsinfo[i].distance )

      if(!useCapsuleRadius ||  ( useCapsuleRadius && lbsinfo[i].distance <= j->capsuleRadius) ) {
        
        wij = 1. / lbsinfo[i].distance;
        /*
        if(wij > 1000.) //cap it
        {
          wij = 1000.;
          lbsinfo[i].distance = 1./wij;
        }
          */
        sumWeights += wij;
        //newVertexPosition += wij * lbsinfo[i].blendPos;
        count++;
      }
      else
        lbsinfo[i].distance = -1;

    }

   // if(count > 0)
   //   v->position = newVertexPosition * (1. / sumWeights);
    //else
    //  v->position =
    if (count > 0)
    {
      ...
      {
        if(lbsinfo[k].distance != -1)
          newVertexPosition += (1. / lbsinfo[k].distance / sumWeights) * lbsinfo[k].blendPos;
      }

      v->position = newVertexPosition;
    }
    else
...
void Mesh::forward_euler(float timestep, float damping_factor) {
  ...
  double laplacian;
  double t = timestep;
  double lambda = damping_factor;

  for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); v++,i++) {
    laplacian = v->laplacian();
    velocities[i] = v->velocity * (1. - t * lambda) + t * laplacian;
    //update using old velocity
    positions[i] = v->offset + t * v->velocity;
  }

  i = 0;
  for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); v++,i++) {
    v->velocity = velocities[i];
    v->offset = positions[i];
  }
...
void Mesh::symplectic_euler(float timestep, float damping_factor) {
  ...
  for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); v++,i++) {
    ...
    positions[i] = v->offset + t * velocities[i];
  }
