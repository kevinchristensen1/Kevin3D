...
void Skeleton::reachForTarget(map<Joint*, Vector3D> targets, double time) {
  ...
  double diff = 100.;
  double deltaT = 0.01;
  Vector3D newAngle, q, e, grad;
  Joint *j, *i;
  while(iter < maxNumIter && std::abs(diff) > epsilon )
  {
    diff = 0;
    for (map<Joint*, Vector3D>::iterator it=targets.begin(); it!=targets.end(); ++it)
    {
      j = it->first;
      q = it->second;
      j->calculateAngleGradient(j, q);
      e = j->getEndPosInWorld() - q;
      diff += e.norm();
    }
    for (vector<Joint*>::iterator it2 = joints.begin(); it2 != joints.end(); ++it2)
    {
      i = *it2;
      grad = i->ikAngleGradient;
      newAngle = i->getAngle(time) - deltaT * grad;
      i->setAngle(time, newAngle);
      i->ikAngleGradient = Vector3D(); //reset it to 0
    }
