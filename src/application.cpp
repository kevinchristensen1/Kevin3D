...
void Application::render() {
  ...
  if(pickDrawCountdown < 0) {
    ...
    if(action == Action::Pose && scene->has_selection() && scene->selected.object->getInfo()[0] != string("JOINT") ) 
      scene->getHoveredObject(p); //Kevin Christensen: when moving pose, don't grab the other poses!!
