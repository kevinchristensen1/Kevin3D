...
float Vertex::laplacian() const {
  ...
  HalfedgeCIter h = halfedge();
  //HalfedgeCIter hiter = h;
  VertexCIter i = h->vertex();
  VertexCIter j;
  Vector3D ip, p0,p1,p2,v0,v1,v2,v3, dp, cp;
  double alpha, beta, sum = 0.;
  ip = i->position;
  ...
  do
  {
    j = h->next()->vertex();
    p0 = j->position;
    p1 = h->next()->next()->vertex()->position;
    p2 = h->twin()->next()->next()->vertex()->position;
    ...
    alpha = dot(v0,v1) / (cross(v0,v1)).norm();
    beta = dot(v2,v3) / (cross(v2,v3)).norm();

    sum += (alpha + beta)*(j->offset - i->offset);
