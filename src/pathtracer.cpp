...
#endif
/* //Fixes white dots, also change ray_trace to include bool option
  Spectrum L_out = Spectrum();
  if(isdelta || r.depth == 1)
  ...
  */
  ...
  if (!isect.bsdf->is_delta()) {
    ...
    for (SceneLight* light : scene->lights) {
      ...
      for (int i = 0; i < num_light_samples; i++) {
        ...
        Vector3D so = hit_p + 1000.*EPS_D * dir_to_light;
        Ray shadow = Ray( so , dir_to_light);
  ...
  double pterm =  min(max(1. - f.illum()*PI ,EPS_D), 1.);
  // (3) evaluate weighted reflectance contribution due 
  // to light from this direction
  pdf =  max(min(1.f,pdf), (float) EPS_D);
  if( (double)(std::rand()) / RAND_MAX >= pterm && r.depth < max_ray_depth )
  //if( r.depth < max_ray_depth ) 
  {
    //numRays++;
    Vector3D owi = o2w*wi;
    Lr += (f * trace_ray( Ray(hit_p + owi*EPS_D*1000.,  owi , (int) r.depth+1)) ) * abs( wi.z) * ( 1. / (pdf*(1.-pterm)));
  }
//}
return L_out + Lr;
}
...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  ...
  if (num_samples == 1)
  {
    ...
    p += Vector2D(0.5,0.5);
    p.x /= (double) sampleBuffer.w;
    p.y /= (double) sampleBuffer.h;
  ...
  else
  {
    ...
    for (int i = 0; i < num_samples; i++)
    {
      ...
      p.y /= (double) sampleBuffer.h;
      //cout << p << endl;
      Iavg += trace_ray(camera->generate_ray(p.x, p.y));
    }
    return Iavg * (1./(double) num_samples);
